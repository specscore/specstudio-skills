...
2. Computes the next executable batch (tasks with all predecessors `**Status:** complete`, own status `planning` — which then transition to `queued` as they become batch-eligible).
...
8. After user approval and the required commit exists, advances to the next batch. Repeats until all tasks are `**Status:** complete`.
