...
description: |
  ...
  Skips `type: human` entries (manual invocation cannot suspend for human
  approval); composes the verdict from surviving `type: ai` entries under
  AND-composition. Supports single-artifact, multi-artifact, and recursive
  tree-wide invocation; `--against REF` controls the diff baseline supplied
  to AI reviewers (default `HEAD`). No persistence, no events, no grade —
  output is inline terminal text only.
  Triggers: "specstudio:review", "/review", "review this", "re-review this".
aliases: [review]
---

# Review

Manual surface for the [`reviewer-gates`](../../spec/features/reviewer-gates/README.md) dispatch pipeline. Implements the [Manual Review Command Feature](../../spec/features/manual-review-command/README.md) (Phase 1 of the [`manual-review-and-score-commands`](../../spec/ideas/manual-review-and-score-commands.md) Idea).
...
- **Emitting a SpecStudio event.** Persistence and event emission belong to the deferred Phase 2 `/score` Feature. (`REQ: no-file-writes`.)
...
```
specstudio:review [PATHS...] [-r|--recursive] [--against REF] [--verbose] [--yes]
```
...
The skill writes nothing in `spec/`; emits no events. The single user-facing side effect is the terminal output and the exit code.
...
Before declaring the implementation complete on any change to this skill, verify (against [the Manual Review Command Feature](../../spec/features/manual-review-command/README.md)):
...
- Computing a grade, badge, or persisted report (those are Phase 2 `/score` — a separate Feature).
- Emitting any SpecStudio event.

## References

- [Manual Review Command Feature](../../spec/features/manual-review-command/README.md) — the contract this skill implements; every REQ and AC traces from here.
...
- [`manual-review-and-score-commands` Idea](../../spec/ideas/manual-review-and-score-commands.md) — the source Idea. Phase 1 = this skill (`/review`); Phase 2 (`/score`) is a separate, deferred Feature.
