...
7. **Creation-class skills delegate canonical subtree bootstrapping to the CLI.** When `specstudio:ideate`, `specstudio:specify`, `specstudio:plan`, or `specstudio:sidekick` creates the first artifact in its subtree, the matching `specscore * new` command owns creation of required directories and indexes. Skills may tell the user the CLI bootstrapped extra paths, but they MUST NOT carry or hand-write index templates.
