...
**Master plan.** Source it from the Idea (`**Source:** idea:<slug>`), not a Feature. Its tasks each delegate to a sub-plan via a `**Sub-Plan:** <plan-ref>` task field (a same-repo slug or a cross-repo `<repo-slug>:<plan-slug>` reference) **instead of** a `**Verifies:**` line — a master coordinates, it does not verify ACs directly. The master MUST NOT carry a `**Parent:**` line (it is the root). Scaffold it as an ordinary single-file idea-sourced plan (`specscore plan new <slug> --idea <slug>`): the AC-coverage (`P-001`) and AC-reference (`P-002`) rules do not apply to idea-sourced plans, so it lints clean. *(On CLI versions before idea-sourced/source-less single-file plans were supported — where `P-002` still rejects them — fall back to a **directory-form** plan at `spec/plans/<slug>/README.md`, which the `P-001`–`P-005` rules skip.)*
