...
Make `name:` optional in the loader/validator (Step 3a): when omitted, the entry's *effective* name defaults to its `type:` value. Recompute the uniqueness check (Step 3, current "names unique") over effective names, so two same-type entries that both omit `name:` collide and are refused. Forward addition from the `noop`→`auto-approve` rename revision; lands with the plugin release that ships the rename (the rename is inert until republish).
