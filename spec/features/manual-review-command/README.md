# Feature: Manual Review Command

> [SpecScore.**Studio**](https://specscore.studio): | [Explore](https://specscore.studio/app/github.com/specscore/specstudio-skills/spec/features/manual-review-command?op=explore) | [Edit](https://specscore.studio/app/github.com/specscore/specstudio-skills/spec/features/manual-review-command?op=edit) | [Ask question](https://specscore.studio/app/github.com/specscore/specstudio-skills/spec/features/manual-review-command?op=ask) | [Request change](https://specscore.studio/app/github.com/specscore/specstudio-skills/spec/features/manual-review-command?op=request-change) |
...
Defines `specstudio:review` — a manual, user-facing skill that re-invokes the configured [`reviewer-gates`](../reviewer-gates/README.md) dispatch pipeline against one or more SpecScore artifacts (Ideas, Features, Plans), outside the producer-skill exit context. The skill is a thin wrapper: it carries no reviewer logic of its own, defers entirely to `gates.<stage>.reviewers` in `specscore.yaml`, and surfaces per-artifact `Approved | Issues Found` verdicts plus inline findings. No file writes, no persistence, no grade computation — those land in the Phase 2 `/score` Feature, scoped separately.
...
The skill MUST NOT write or modify any file in `spec/`, `.specscore/`, or any other repo path. It MUST NOT emit any SpecStudio event. Persistence and event emission are deferred to the Phase 2 `/score` Feature, scoped separately.
...
- **No persistence layer.** Output is inline terminal text only. The `/score` Phase 2 Feature will introduce `spec/_score/<artifact-slug>-<sha>.md` persistence; this Feature deliberately defers that.
...
- **Skill implementation lives at.** `skills/review/SKILL.md` (parallels `skills/specify/SKILL.md`). The Feature artifact defines the contract; the skill file is its implementation.
...
| [Archived Review Feature](../skills/review/README.md) | Per `reviewer-gates#review-feature-archival`, the old `review` pipeline-step Feature is archived. This new Feature reuses the `specstudio:review` trigger name and the `skills/review/` directory path. Naming collision is acknowledged in the source Idea; risk is mitigated by the archived Feature's clear archive-reason pointer. |
| `/score` Phase 2 Feature (not yet specified) | This Feature is Phase 1 of a two-phase rollout. The Phase 2 `/score` Feature will wrap this skill's pipeline (verdict + findings) and add grade computation, report persistence, badge injection, and aggregate output. The Phase 1 Feature MUST be implementable and useful on its own without any Phase 2 work — see the source Idea's MVP scope. |
...
**Given** the skill is installed and a `specstudio:review` command is registered in the platform's skill registry,
**When** the user issues any of the strings `specstudio:review`, `/review`, `review this`, or `re-review this` against an artifact path,
...
**Given** the user invokes `specstudio:review spec/ideas/some-idea.md --not-a-real-flag`,
...
**Given** the user invokes `specstudio:review` with no `PATHS` argument and without `-r`,
...
**Given** the user invokes `specstudio:review -r` with no `PATHS` argument in a repo containing `spec/ideas/foo.md`, `spec/features/bar/README.md`, and `spec/plans/baz.md`,
**When** the skill resolves paths,
...
**Given** the user invokes `specstudio:review spec/features/bar` and the directory `spec/features/bar/` contains a `README.md`,
**When** the skill resolves paths,
...
**Given** the user invokes `specstudio:review spec/features/empty-dir` where the directory exists but contains no `README.md`,
**When** the skill resolves paths,
...
**Given** the user invokes `specstudio:review 'spec/features/nonexistent-*/README.md'` and zero files match the pattern,
**When** the skill resolves paths,
...
**Given** the user invokes `specstudio:review spec/features/bar/README.md spec/features/ -r` and `spec/features/bar/README.md` is reachable both as the explicit first entry and via recursive descent from `spec/features/`,
**When** the skill resolves paths,
...
**Given** the user invokes `specstudio:review spec/ideas/foo.md spec/features/bar/README.md spec/plans/baz.md spec/research/notes.md`,
...
**Given** the user invokes `specstudio:review spec/features/README.md`,
...
**Given** `specscore.yaml` configures `gates.specify` but does NOT configure `gates.ideate`, and the user invokes `specstudio:review spec/ideas/foo.md`,
...
**Given** `gates.specify.reviewers` contains one `type: ai` entry followed by one `type: human` entry, and the user invokes `specstudio:review spec/features/bar/README.md`,
...
**Given** `gates.ideate.reviewers` contains only `type: human` entries (no AI reviewers), and the user invokes `specstudio:review spec/ideas/foo.md`,
**When** the skill dispatches reviewers for that artifact,
...
**Given** the user invokes `specstudio:review spec/features/bar/README.md` without `--against`, and the artifact has uncommitted working-tree edits relative to `HEAD`,
...
**Given** the user invokes `specstudio:review spec/features/bar/README.md --against main`,
**When** the skill dispatches the `type: ai` reviewers for that artifact,
...
**Given** the user invokes `specstudio:review spec/features/bar/README.md --against does-not-exist-ref`,
...
**When** the user immediately afterward runs `specstudio:review spec/features/bar/README.md` against the same `HEAD`,
...
**Given** the resolved-artifact count after `specstudio:review spec/ -r` is 11, no `--yes` flag was supplied, and the user has not yet responded,
...
**Given** the resolved-artifact count after `specstudio:review spec/features/ -r` is 10 and no `--yes` flag was supplied,
**When** the skill completes path resolution,
...
**Given** the resolved-artifact count after `specstudio:review spec/ -r --yes` is 50,
**When** the skill completes path resolution,
...
**When** `specstudio:review` runs through them,
...
**Given** the user invokes `specstudio:review spec/features/bar/README.md` (resolved count exactly 1) without `--verbose`,
...
**Given** any invocation of `specstudio:review` with any combination of arguments,
...
**Given** `specscore.yaml` has no top-level `gates:` key at all and the user invokes `specstudio:review spec/features/bar/README.md`,
...
- **`/score` and grade computation** — Phase 2; the report-file shape (`spec/_score/<artifact-slug>-<sha>.md`), the weighted A–F rubric, the per-axis breakdown, and aggregate output across multiple artifacts all need design that should not block `/review` shipping. A separate Feature will be specified once Phase 1 is dogfooded.
- **Badge injection into artifacts** — depends on `/score`; comes after Phase 2. This includes per-artifact badges, per-sub-artifact badges with `-r`, and root-index badges in `spec/README.md`.
- **Persistence of `/review` output** — no `spec/_review/...` files in MVP. Output is inline terminal text only. Persistence and event emission are explicit `/score` concerns.
- **Event emission** — the skill emits no event in MVP. The Phase 2 `/score` Feature may emit `score.completed`; that decision is owned there.
...
- **Auto-running `/review` on every `specscore spec lint`** — performance and noise risk per the source Idea. The skill is signal, not gate; users invoke it explicitly.
...
- **Behavior when an artifact lints dirty at the time of `/review`** — the skill currently dispatches reviewers without first running `specscore spec lint`. Lint failures will surface inside reviewer output (the AI reviewer is expected to call them out), but a lint-clean precondition is not enforced. Whether to short-circuit on lint failure (refuse to dispatch reviewers on a lint-dirty artifact) is deferred to dogfood signal.
- **Surfacing of skipped artifacts in single-artifact mode** — when the user invokes `specstudio:review` on exactly one artifact and that artifact is skipped (e.g., it's an index file or `spec/research/notes.md`), the output is the per-artifact section with verdict `Skipped`. Whether the skill should additionally print a recommendation ("this artifact has no gate — did you mean a different path?") is left to implementation polish.
