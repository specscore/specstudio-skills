...
When the strict gate passes (`unanimous Builders + Customers + no adversary veto + median confidence ≥ medium + cost & complexity ≤ 🟡`), the worker auto-invokes `specstudio:specify` and `superpowers:writing-plans` to draft the Feature spec and implementation plan — and stops. Code is never written autonomously.
...
**Numerical 1–9 scoring.** Looks precise, isn't. *Lost because:* unanchored numeric scales aren't calibrated across roles or runs. Replaced with a 3-step ordinal (🟢/🟡/🔴) plus a required *Confidence* level (low/med/high) and a one-sentence *Strongest argument* per expert.
...
- **Veto confidence threshold.** Current rule: high-confidence adversary `should-not-implement` triggers the gate. Should *medium*-confidence vetoes also block, or only flag? Conservative is medium-blocks; aggressive is high-only.
