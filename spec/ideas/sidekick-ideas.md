...
- **New Features this would create:**
  ...
  - `skills/consilium/auto-promote` (Phase 2) — the strict-gate promotion chain that invokes `specstudio:specify` and `superpowers:writing-plans`.
...
- **Roster override surface.** Per-project `synchestra.yaml` is decided. Open: do we also allow per-invocation overrides (`/consilium --without security,ops`)? Pro: useful for cost control on small seeds. Con: undermines the "fixed default" guarantee.
...
- **Two-way back-links from source artifact to generated seeds.** Future Feature (post-Phase-0): extend the SpecScore Feature/Idea/Plan specifications themselves to formally include a `## Sidekick Seeds Generated` section listing seeds whose `captured_during` points at the artifact. Auto-managed by `specscore spec lint --fix` (same pattern as ideas-index sync and `Promotes To` reverse-link sync). Cross-repo work: extends [specscore.md](https://specscore.md/) schema and the `specscore` CLI scaffolders. Not blocking Phase 0 (the data is already in the seed's `captured_during`); adds discoverability ("looking at this artifact, what sideline ideas did we surface?") and gives the Phase 1 consilium richer sibling-seed context within the same source context.
