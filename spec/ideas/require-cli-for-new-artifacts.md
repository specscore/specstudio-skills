...
| Might-be-true | Authors will not miss an inline schema reference once it is removed, because running the scaffold once shows the shape | Convert the three MVP skills; ask a maintainer to author an artifact with the converted skills and report whether the missing inline schema hurt |
...
- Where on specscore.md do canonical templates live, and what is the fetch contract (URL scheme per artifact type, caching, auth/none)?
