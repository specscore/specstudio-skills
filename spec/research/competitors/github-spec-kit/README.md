# GitHub Spec Kit vs. the orchestrator Ecosystem — Comparative Analysis
...
- Sibling repos in `specscore/`: `specscore` (open spec format, CC BY 4.0; CLI at `specscore-cli`, Apache-2.0) and `orchestrator` (Go CLI + MCP + Hub, Apache-2.0)
...
| `/speckit.implement` (run the tasks) | `orchestrator` daemon + Hub | **Different jobs.** Spec Kit = single-session local; Synchestra = multi-agent async. Synchestra wins here, Spec Kit doesn't try. |
...
- A user fully bought into the orchestrator doesn't need Spec Kit for anything Spec Kit uniquely provides — every step has a Synchestra-side equivalent.
...
- `after_tasks` hook with `optional: true, prompt: "Enqueue into the orchestrator for parallel/async execution?"` — meets the user where Spec Kit leaves them
